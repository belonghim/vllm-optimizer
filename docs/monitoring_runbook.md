...
Run the comprehensive unit test for the metrics endpoint:

```bash
PYTHONPATH=. python -m pytest backend/tests/test_dev_metrics_endpoint.py -v
```

**Expected**: All tests pass. This test mocks `generate_metrics()` and validates Prometheus format, metric presence, and histogram structure.

**Evidence**: Capture output to `.sisyphus/evidence/task-6-pytest-output.txt`

### 2. Integration Test (No Mocks)

Verify the full stack without mocking:

```bash
PYTHONPATH=. python -m pytest backend/tests/test_integration_metrics_e2e.py -v
```

**Expected**: Test passes, confirming that:
...
### 4. OpenShift Configuration Validation

Ensure ServiceMonitor path aligns with the endpoint:

```bash
python openshift/validate_monitoring_config.py
```

**Expected output**: `ServiceMonitor path (/api/metrics) found in 05-monitoring.yaml`

**What it checks**: The script scans `openshift/base/05-monitoring.yaml` for `path: /api/metrics` in the ServiceMonitor endpoints.
...
3. Run `debug_routes.py` to list all registered routes; ensure `/api/metrics` appears.
...
**Steps**:
1. Validate config: `python openshift/validate_monitoring_config.py`
...
- `backend/tests/test_dev_metrics_endpoint.py` — Unit tests for `/api/metrics` with mocked output.
- `backend/tests/test_integration_metrics_e2e.py` — End-to-end integration test.
...
| Action | Command |
|--------|---------|
| Run unit tests | `pytest backend/tests/test_dev_metrics_endpoint.py -q` |
| Run integration test | `pytest backend/tests/test_integration_metrics_e2e.py -q` |
| Validate monitoring config | `python openshift/validate_monitoring_config.py` |
| List registered routes | `python debug_routes.py` |
