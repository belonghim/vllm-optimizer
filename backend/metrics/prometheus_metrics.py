...
gpu_cache_usage_perc_metric = Gauge(
    "vllm_optimizer_gpu_cache_usage_perc", "GPU KV cache usage percentage (mirrored by optimizer)", registry=_registry
)

gpu_utilization_metric = Gauge(
    "vllm_optimizer_gpu_utilization", "GPU utilization percentage (mirrored by optimizer)", registry=_registry
)
...
tuner_trials_total = Counter(
    "vllm_optimizer_tuner_trials_total", "Total number of auto-tuning trials by status", ["status"], registry=_registry
)
