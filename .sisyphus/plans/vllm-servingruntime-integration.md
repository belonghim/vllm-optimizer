...
- [ ] InferenceService Ready 상태 (`oc get isvc llm-ov -n vllm` → READY=True)
...
- [ ] Auto Tuner가 vLLM ConfigMap 패치 성공 (end-to-end test 완료)
...
- 1. Validate vLLM ConfigMap structure in 02-config.yaml
  ...
  **Must NOT do**:
  - ConfigMap 구조를 임의로 수정하지 않음 (실제 vLLM 이미지 문서 기준)
...
- 2. Create vLLM-Role + RoleBinding for optimizer-backend SA
  ...
  **Commit**: YES
  ...
  - Files: `openshift/dev-only/vllm-rbac.yaml`
...
- 3. Update kustomization.yaml with new resources
  ...
  **References**:
  - `openshift/dev-only/kustomization.yaml` — current resources list
  - `openshift/overlays/dev/kustomization.yaml` — vllm-optimzer dev overlay bases
  ...
  **Commit**: YES
  ...
  - Files: `openshift/dev-only/kustomization.yaml`
  - Pre-commit: `git diff --check openshift/dev-only/kustomization.yaml`
...
- 4. Create vLLM ServingRuntime YAML
  ...
  **Acceptance Criteria**:
  - [ ] YAML 파일 `openshift/dev-only/vllm-runtime.yaml` 생성됨
  ...
  **QA Scenarios**:
  ...
  Scenario: Verify ServingRuntime YAML syntax and required fields
    ...
    Steps:
      1. Validate YAML: `oc create --dry-run=client -f openshift/dev-only/vllm-runtime.yaml`
  ...
  Scenario: Verify ServingRuntime can be applied without conflicts
    ...
    Steps:
      1. `oc kustomize openshift/dev-only | oc apply --dry-run=client -f -`
  ...
  **Commit**: YES
  ...
  - Pre-commit: `oc create --dry-run=client -f openshift/dev-only/vllm-runtime.yaml`
...
- 5. Create vLLM InferenceService YAML
  ...
  **QA Scenarios**:
  ...
  Scenario: Validate InferenceService YAML
    ...
    Steps:
      1. `oc create --dry-run=client -f openshift/dev-only/vllm-inferenceservice.yaml`
  ...
  Scenario: Check that InferenceService references correct runtime
    ...
    Steps:
      1. `grep -A2 'runtime:' openshift/dev-only/vllm-inferenceservice.yaml`
      2. `grep 'name:' openshift/dev-only/vllm-runtime.yaml`
  ...
  **Commit**: YES
  ...
  - Files: `openshift/dev-only/vllm-inferenceservice.yaml`
  - Pre-commit: `oc create --dry-run=client -f openshift/dev-only/vllm-inferenceservice.yaml`
...
- 6. Deploy vLLM resources with Kustomize (automated via deploy.sh)
  ...
  **QA Scenarios (가이드에 포함)**:
  ...
  Scenario: Verify vLLM InferenceService is Ready
    ...
    Steps:
      1. `oc get isvc llm-ov -n vllm -o jsonpath='{.status.conditions[?(@.type=="Ready")].status}'`
  ...
  **Commit**: YES
  - Message: `feat(deploy): automate vLLM resource deployment and SCC assignment`
  - Files: `deploy.sh`
  - Pre-commit: `git diff --check deploy.sh`
...
- 7. Create vLLM ServiceMonitor + PrometheusRule (06-vllm-monitoring.yaml)
  ...
  **What to do**:
  ...
  - **ServiceMonitor**:
    ...
    - `spec.endpoints`:
      ...
      - `scheme: http`
  - **PrometheusRule**:
    - vLLM 성능 알람: `VLLMHighP99Latency`, `VLLMHighQueueDepth`, `VLLMKVCacheSaturation`, `VLLMLowThroughput` — **메트릭 이름은 이미 언더스코어 형식 사용 중**
  ...
  **Must NOT do**:
  ...
  - **vLLM 메트릭 이름을 urgent-fixes와 다르게 정의** — 이미 언더스코어 형식으로 일치함
  ...
  **Acceptance Criteria**:
  - [x] `ServiceMonitor` resource created with correct target (vLLM service)
  - [x] `PrometheusRule` with expressions that match vLLM metrics (`vllm_num_requests_waiting`, `vllm_gpu_cache_usage_perc`, etc.) - **already using underscores**
  - [x] `Port` name matches vLLM service's port name (usually `http`)
  - [x] Namespace `vllm` has label `openshift.io/cluster-monitoring: "true"` (05-monitoring.yaml 참고)
  ...
  **Commit**: YES
  ...
  - Files: `openshift/dev-only/06-vllm-monitoring.yaml`
  - Pre-commit: `oc create --dry-run=client -f openshift/dev-only/06-vllm-monitoring.yaml`
...
- 8. Create vLLM NetworkPolicy (allow optimizer-backend access)
  ...
  **Commit**: YES
  ...
  - Files: `openshift/dev-only/vllm-networkpolicy.yaml`
  - Pre-commit: `oc create --dry-run=client -f openshift/dev-only/vllm-networkpolicy.yaml`
...
- 9. Verify ServiceMonitor scraping
  ...
  **What to do**:
  ...
  - Prometheus UI에서 vLLM service가 scrape되고 있는지 확인
...
- 13. Write integration test guide (docs/integration_test_guide.md)
  ...
  **Commit**: YES
  - Message: `docs: add integration test guide for vLLM`
...
oc exec $BACKEND_POD -n vllm-optimizer-dev -- curl -s http://llm-ov-predictor.vllm.svc.cluster.local:8080/v1/models | jq -r '.object' | grep -q list
```
