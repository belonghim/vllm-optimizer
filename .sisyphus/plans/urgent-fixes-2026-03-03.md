...
**Acceptance Criteria**:
- [ ] vllm-optimizer-backend SA에 `cluster-monitoring-view` RoleBinding 존재
- [ ] SA에 `pods/log` 접근 권한 확인 (필요시)
- [ ] `oc auth can-i --list --as=system:serviceaccount:vllm-optimizer-dev:vllm-optimizer-backend` 출력에 필요한 권한 포함
...
**Acceptance Criteria**:
- [ ] 모든 `.yaml` 파일에서 `vllm:` 패턴 사라짐
- [ ] `VLLM_QUERIES`의 모든 PromQL 쿼리가 `vllm_` prefix 사용
- [ ] `grep -rE "vllm:[a-zA-Z0-9_]+" .` 결과 0건
- [ ] `grep -rE "vllm_[a-zA-Z0-9_]+" .` 결과 10+ 건
- [ ] `python -m py_compile backend/services/metrics_collector.py` 성공
...
**Acceptance Criteria**:
- [ ] `self._state_lock` 및 `self._subscribers_lock`가 `__init__`에 정의됨
- [ ] 모든 `_state` mutations (lines 139-143, 172)가 `async with self._state_lock:`로 보호됨
- [ ] 모든 `_subscribers` 접근 (lines 46, 50, 53)가 `async with self._subscribers_lock:`로 보호됨
- [ ] `_broadcast()`가 lock 하에 snapshot 복사 후 iteration (lines 52-54)
...
- [ ] `Access-Control-Allow-Methods` 헤더 추가됨
- [ ] `Access-Control-Allow-Headers` 헤더 추가됨
- [ ] OPTIONS preflight 핸들러 블록 존재 (`return 204`)
- [ ] Nginx 설정 문법 오류 없음 (`nginx -t` 성공)
...
**Acceptance Criteria**:
- [ ] `/health` 엔드포인트에 `deep` query parameter 지원
- [ ] deep=1 시 Prometheus 및 K8s API connectivity 검사 수행
- [ ] 종속성 실패 시 503 응답
- [ ] 응답 JSON에 `dependencies` 키 포함
- [ ] 기본 health check (deep 없음) 여전히 빠름 (<1초)
...
- [ ] `oc apply --dry-run=client -f openshift/base/04-frontend.yaml` 성공
...
- [ ] `oc apply --dry-run=client -f openshift/base/03-backend.yaml` 성공
...
- [ ] ServiceMonitor YAML syntax valid
...
**Acceptance Criteria**:
- [ ] `deploy.sh`에 `oc rollout status` 두 번 추가됨 (backend, frontend)
- [ ] `oc wait --for=condition=Ready` 두 번 추가됨
- [ ] 각 대기 단계 후 success 메시지 출력
- [ ] `--timeout` 파라미터 명시 (5m for rollout, 300s for pods)
- [ ] 배포 스크립트 실행 시 rollout 완료까지 대기
...
- [ ] **Wave 1 Complete**: Tasks 1-6 (Foundation)
- [ ] **Wave 2 Complete**: Tasks 7-12 (Config & Logic)
- [ ] **Wave 3 Complete**: Task 13-14 (Integration)
- [ ] **Final Audit**: F1-F4 all APPROVE
- [ ] **Git Commit**: Single coherent commit with all changes
- [ ] **Evidence**: `.sisyphus/evidence/` complete with 14 tasks × 2+ scenarios each
- [ ] **Documentation**: `CHANGELOG.md` updated with emergency fixes
